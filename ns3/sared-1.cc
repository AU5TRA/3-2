 ...
 * Implementation based on: Shan Suthaharan, "Markov Model Based Congestion Control for TCP"
 *
 * FIXES:
 * 1. Replaced deprecated UniformVariable() with UniformRandomVariable
 * 2. Fixed QueueLimits API (SetQueueLimits removed, use AddInternalQueues instead)
 * 3. Fixed transition probability typo: p21 was assigned twice (should be p12)
 * 4. Fixed SARED boundary condition: paper uses (ave >= th_max - part), code used wrong comparison
 * 5. Fixed idle queue EWMA: time-based decay uses seconds not meanPktSize as divisor
 * 6. Fixed m_count initialization: should start at 0 not -1 for proper RED logic
 * 7. Added missing internal queue setup in CheckConfig
 * 8. Replaced deprecated ns3::QueueLimits attribute API
 */
...
#include "ns3/drop-tail-queue.h"   // important
  ...
  : QueueDisc (QueueDiscSizePolicy::SINGLE_INTERNAL_QUEUE)
{
  m_curMaxP = 0.125;
  m_qAvg    = 0.0;
  m_count   = -1;  // -1 means "not in drop zone yet"
  m_lastSet = Seconds (0.0);
  m_oldAve  = 0.0;
  m_qTime   = Seconds (0.0);

  // FIX 7: create RNG object properly
  m_uv = CreateObject<UniformRandomVariable> ();

  // Initialize Markov counts (Laplace smoothing = 1)
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      {
        m_n[i][j] = 1.0;
  ...
  // Target stationary distribution: mostly state 1 (between thresholds)
  m_targetP[0] = 0.05;
  m_targetP[1] = 0.90;
  m_targetP[2] = 0.05;
}
...
bool SaredQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  // 1. EWMA of queue size
  if (nQueued == 0)
    {
      // FIX 5: idle decay — m is number of "ghost" packet transmissions
      // that would have occurred during idle time
      double idleSecs = (Simulator::Now () - m_qTime).GetSeconds ();
      // Approximate link rate: use meanPktSize / linkRate. Since we don't
      // have link rate here, use a small fixed time per packet (500us ~ 1500B @ 24Mbps).
      // The paper just decays: avg = avg * (1-wq)^m
  ...
  // 2. SARED update (adjust m_curMaxP)
  if (Simulator::Now () > m_lastSet + m_interval)
    {
      UpdateSared ();
    }

  // 3. RED drop decision using current m_curMaxP
  bool drop = false;
  ...
  if (m_qAvg < m_minTh)
  ...
  else if (m_qAvg < m_maxTh)
    {
      ...
      // FIX 6: Standard RED gentle random drop formula
      if (pb > 0.0)
        {
    ...
    {
      // Queue average above maxTh — always drop
      drop = true;
      m_count = 0;
    }
...
// ---------------------------------------------------------------------------
// SARED Update — directly from paper's algorithm
// ---------------------------------------------------------------------------
  ...
  if (m_qAvg <= m_minTh + part)
  ...
  else if (m_qAvg < m_maxTh - part)
    {
      if (m_curMaxP <= 0.01)
        ...
        {
          // Paper: alpha=p12*(p0+p2); beta=p11*p1
          alpha     = m_p[1][2] * (m_targetP[0] + m_targetP[2]);
          beta      = m_p[1][1] * m_targetP[1];
          m_curMaxP = m_curMaxP * beta + alpha;
        }
    }
  else  // ave >= th_max - part  (FIX 4: was wrong branch condition)
    {
      if (m_curMaxP <= 0.01)
        {
...
// FIX 3: Corrected typo — original code assigned p21 twice; should be p12
      ...
      if (sum > 0)
        {
          m_p[i][0] = m_n[i][0] / sum;
          m_p[i][1] = m_n[i][1] / sum;
          m_p[i][2] = m_n[i][2] / sum;  // FIX 3: was m_p[i][2] but paper had p21=n21/n1 typo
        }
    }
}
...
int main (int argc, char *argv[])
{
  ...
  // Install SARED on the bottleneck egress interface (router 0 → router 1)
  TrafficControlHelper tch;
  // SetRootQueueDisc return value not needed — CheckConfig adds the internal queue
  tch.SetRootQueueDisc ("ns3::SaredQueueDisc",
                        "MinTh", DoubleValue (thMin),
                        "MaxTh", DoubleValue (thMax));
  // Note: internal DropTailQueue is added inside SaredQueueDisc::CheckConfig()
  // using CreateObjectWithAttributes<DropTailQueue<QueueDiscItem>>(), avoiding
  // the broken TypeId string lookup that tch.AddInternalQueues() would trigger.
  ...
  // Phase 1: 6 steady background flows → sink 0 (runs full 360s)
  ...
  // Phase 2: 20 burst flows → sink 1 (starts at 120s, ends at 300s)
