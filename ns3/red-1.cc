 ...
 * Enhanced Version with Logging and Tracing
 */
...
// {
//   NS_LOG_DEBUG ("Time: " << Simulator::Now ().GetSeconds () 
//                 << "s - Packet Sent - Size: " << packet->GetSize () 
//                 << " bytes");
// }

// Callback for packet reception
// static void
// OnPacketReceived (Ptr<const Packet> packet, const Address &address)
// {
//   NS_LOG_DEBUG ("Time: " << Simulator::Now ().GetSeconds () 
//                 << "s - Packet Received - Size: " << packet->GetSize () 
//                 << " bytes");
// }
...
{
  // --- 1. EXTREME CONGESTION SETTINGS (GUARANTEED DROPS!) ---
  double thMin = 10.0;   // Start dropping at 10 packets
  double thMax = 30.0;   // Max drop probability at 30 packets
  uint32_t bufferSize = 100;  // Buffer for 100 packets
  double wq = 0.002;
  double maxP = 0.3;     // 30% max drop probability
  double lInterm = 50.0;
  
  uint32_t packetSize = 1024; 
  double simDuration = 30.0;  // Longer simulation
  uint32_t numSources = 20;   // MANY sources
  
  // Link parameters
  std::string edgeRate = "10Mbps";
  std::string bottleneckRate = "500Kbps";  // VERY NARROW bottleneck
  std::string sourceRate = "200Kbps";      // Each source sends 200Kbps
  ...
  // Calculate congestion ratio (parse strings manually to avoid std::stod issues)
  // Extract numeric part from strings like "500Kbps"
  ...
  double bottleneckBw = atof(bottleneckNumStr.c_str());
  double sourceBw = atof(sourceNumStr.c_str());
  double totalOffered = numSources * sourceBw;
  double congestionRatio = totalOffered / bottleneckBw;
  ...
  NS_LOG_INFO ("Starting RED Queue Simulation");
  NS_LOG_INFO ("Configuration: MinTh=" << thMin << ", MaxTh=" << thMax 
               << ", BufferSize=" << bufferSize << ", QW=" << wq 
               << ", MaxP=" << maxP);
  NS_LOG_INFO ("CONGESTION SETUP:");
  NS_LOG_INFO ("  Sources: " << numSources << " @ " << sourceRate << " each");
  NS_LOG_INFO ("  Total Offered Load: " << totalOffered << " Kbps");
  NS_LOG_INFO ("  Bottleneck Capacity: " << bottleneckBw << " Kbps");
  NS_LOG_INFO ("  CONGESTION RATIO: " << congestionRatio << ":1");
  
  if (congestionRatio < 1.5) {
    NS_LOG_WARN ("WARNING: Low congestion ratio (" << congestionRatio 
                 << ":1) - may not see drops!");
    NS_LOG_WARN ("Recommendation: Increase numSources or decrease bottleneckRate");
  }
  ...
  NS_LOG_INFO ("Created topology: " << numSources << " sources, " << numSources << " sinks, 2 routers");
  ...
  // NARROW BOTTLENECK - THIS CREATES CONGESTION!
  ...
  NS_LOG_INFO ("Configured links: Edge=" << edgeRate << "/1ms, Bottleneck=" << bottleneckRate << "/20ms");
  ...
  NS_LOG_INFO ("Installed Internet stack");

  // --- 5. Install RED & Connect Traces ---
  ...
  // Connect traces to all queue discs
  for (uint32_t i = 0; i < qDiscs.GetN (); ++i) {
      Ptr<QueueDisc> q = qDiscs.Get (i);
      uint32_t nodeId = bottleneckDevices.Get (i)->GetNode ()->GetId ();
      
      NS_LOG_INFO ("Installing RED queue on Node " << nodeId << " - Connecting traces");

      // Connect core trace sources
  ...
  NS_LOG_INFO ("Configured IP addressing and routing");

  // --- 7. Traffic Generation (UDP for aggressive constant-rate traffic) ---
  uint16_t port = 9;
  
  NS_LOG_INFO ("Configuring " << numSources << " UDP sources...");
  
  // Install applications on all sources
  for (uint32_t i = 0; i < numSources; ++i)
    {
      // UDP Sink
      ...
      // Staggered start to avoid synchronization
      sourceApps.Start (Seconds (1.0 + i * 0.05)); 
      sourceApps.Stop (Seconds (simDuration));
      
      if (i == 0) {
        NS_LOG_INFO ("Sample: Source 0 -> " << sinkAddress << ":" << port 
                     << " @ " << sourceRate);
      }
    }
  
  // Parse bandwidth values for logging
  ...
  double totalBw = numSources * atof(totalBwStr.c_str());
   bottleneckBw = atof(bottleneckBwStr.c_str());
  ...
  // --- 8. Enable Packet Capture (optional) ---
  if (enablePcap)
    {
  ...
  NS_LOG_INFO ("");
  NS_LOG_INFO ("NOTE: Drop rate = Dropped / (Enqueued + Dropped)");
  NS_LOG_INFO ("      This shows what % of arriving packets were dropped by RED");
  NS_LOG_INFO ("");
  ...
  // Get total packets from queue statistics (more accurate)
  ...
  // Total packets attempted = enqueued + dropped
  ...
  // Print final statistics for each queue
  for (uint32_t i = 0; i < qDiscs.GetN (); ++i)
    {
      Ptr<QueueDisc> q = qDiscs.Get (i);
      QueueDisc::Stats stats = q->GetStats ();
      ...
      NS_LOG_INFO ("  Dropped (Bytes): " << stats.nTotalDroppedBytes);
      NS_LOG_INFO ("  Marked: " << stats.nTotalMarkedPackets);
      
      if (stats.nTotalEnqueuedPackets + stats.nTotalDroppedPackets > 0)
        {
          double qDropRate = (double)stats.nTotalDroppedPackets / 
                             (double)(stats.nTotalEnqueuedPackets + stats.nTotalDroppedPackets) * 100.0;
  ...
  NS_LOG_INFO ("Trace files written: red-drops.dat, red-queue-avg.dat, red-statistics.dat");
